...
async function deleteFiles(urls) {
    ...
    const uniq = Array.from(new Set(urls.filter(Boolean).map(u => {
        try {
            ...
            return path_1.default.basename(p.pathname);
        }
        catch (e) {
            return path_1.default.basename(u);
        }
    })));
    await Promise.all(uniq.map(async (file) => {
        ...
        try {
            ...
            await cloudinary_1.default.uploader.destroy(publicId, { resource_type: 'image' }).catch(() => { });
            await cloudinary_1.default.uploader.destroy(publicId, { resource_type: 'video' }).catch(() => { });
            // Note: cloudinary destroy is idempotent; if not found will not throw
            return;
        }
        catch (err) {
            if (attemptedCloudinary)
                console.warn('Cloudinary deletion failed for', file, err);
        }
        // Fallback to local filesystem delete only if explicitly allowed by env
        if (process.env.ALLOW_LOCAL_UPLOADS === 'true') {
            const full = path_1.default.join(uploadsDir, file);
            try {
